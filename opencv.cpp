...
opencv::opencv() : cap(), yolo(new class yolo), qtimer(new QTimer) {
...
void opencv::set_video(QString s) {
    ...
    string month = QString::number(tm->tm_mon+1).toStdString();
    string day = QString::number(tm->tm_mday).toStdString();
    ...
    try {
        ...
        writer.open(filename, VideoWriter::fourcc('M', 'J', 'P', 'G'),
                    videoInfo.fps, cv::Size(videoInfo.width, videoInfo.height),
                    true);
    } catch (cv::Exception &e) {
        qDebug() << e.what();
    }
...
bool opencv::dispose() {
    ...
    cv::resize(mat, mat,
               cv::Size(width > 1920 / 2 ? 1920 / 2 : width,
                        height > 1080 / 2 ? 1080 / 2 : width));
